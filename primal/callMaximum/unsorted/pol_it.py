...
class Pol_it:
    ...
    def build_samples(self, samples_min, samples_max):
        ...
        # samples_mat = np.flip(np.sort(samples_mat, axis=0), axis=0)
        # samples_mat_test = np.flip(np.sort(samples_mat_test, axis=0), axis=0)
        # samples_mat[::-1].sort(axis=0)
        # samples_mat_test[::-1].sort(axis=0)
        # print('diff', la.norm(samples_mat_2 - samples_mat))
        # input()
    ...
    def solve_HJB(self, start_num = None):
        ...
        for i0 in range(start_num, -1, -1):
            ...
            if i0 is not start_num:
                ...
                self.v.c_add_fun_list[self.curr_ind] = self.v.c_add_fun_list[self.curr_ind+1]
