...
def polynomials(pol_deg):
    def create_lambd_fun(index):
        if index == 0:
            ...
            return lambda x: 1*np.poly1d([-1, 1])(x/100)
        elif index == 2:
            return lambda x: 1*np.poly1d([1/2, -2, 1])(x/100)
        elif index == 3:
            return lambda x: 1*np.poly1d([-1/6, 1.5, -3, 1])(x/100)
        elif index == 4:
            return lambda x: 1*np.poly1d([1/24, 2/3, 3, -4, 1])(x/100)
        # def _(x):
            # return pol[index](x)
        # return _
    pol = []
...
class Valuefunction_TT:
    def __init__(self, valuefunction_prename=None, load_existing_list=False, c_name=None):
    ...
    def set_add_fun(self, fun):
        self.add_fun = fun


    def replace_last_pol(self):
        def create_g_batch():
            def _(x):
                return np.maximum(0, 100-x)
            return _
        ex = create_g_batch()
        self.pol[-1] = ex; self.dpol[-1] = ex
    ...
    def solve_linear_HJB(self, data, params):
        ...
        while _n_sweep < n_sweep and rel_val > rel_val_tol and rel_val_test > rel_val_tol or _n_sweep <= 0:
            ...
            # print(V.ranks()) 
            # print('val, old_val', val, old_val, 'rel_val', rel_val, '_n_sweep', _n_sweep, 'frob_norm(v)', xe.frob_norm(V), 'validation', val_test, 'rel_val_test', rel_val_test)
        ...
        print('ind', ind, 'ranks', V.ranks(), 'adapt=', adapt) 
        self.V[ind] = V
    ...
    #        op = np.einsum('ijkl,mnol->ijkmno', op_pre, op_pre)
            # print('op_pre.shape', op_pre.shape)
    ...
    #    Ax = np.einsum('jkli,jkl->i', op_pre, sol_arr_reshape)
        ...
        return w, error1 + 0, current_fun_c
